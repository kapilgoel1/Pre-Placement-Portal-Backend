...
router.get('/retrieve', isAuthenticated, async (req, res) => {
  try {
    ...
    Promise.all([resourceList, numOfResources]).then((response) => {
      res
        .status(200)
        .send({ resourceList: response[0], numOfResources: response[1] });
    });
  } catch (err) {
    console.log(err);
    res.status(500).send();
...
router.get('/retrieveoptimised', isAuthenticated, async (req, res) => {
  try {
    const { skip, limit, ...filterOptions } = req.query;
    var resourceList = ExternalResource.find({ ...filterOptions }).sort({
      createdAt: 'desc',
    });

    Promise.all([resourceList]).then((response) => {
      res.status(200).send({ resourceList: response[0] });
    });
  } catch (err) {
    console.log(err);
    res.status(500).send();
